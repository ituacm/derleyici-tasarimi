...
int main()
{
	...
	size_t tk_ids[] = {
		...
		5, 4, 2, 3, 6, TK_NOTOKEN
	};

	lexer_init(&l, "if test else test test2 123 -1.2 -> - int + >");

	for (size_t i = 0; i < sizeof(tk_ids) / sizeof(size_t); i++) {

		tokenizer_feed(&t, lexer_next(&l));

		struct token tk = tokenizer_next(&t);

		assert(tk.tk_id == tk_ids[i]);

		if (tk.tk_id == TK_INT)
			assert(tk.seminfo.num_int == 123);

		if (tk.tk_id == TK_FLOAT)
			assert(tk.seminfo.num_float == -1.2);

		if (tk.tk_id == TK_IDENT)
	...
	assert(tokenizer_next(&t).tk_id  == 4);
	assert(tokenizer_next(&t).tk_id  == 5);
	assert(tokenizer_next(&t).tk_id  == 5);
	assert(tokenizer_next(&t).tk_id  == 6);
	assert(tokenizer_next(&t).tk_id  == 5);
